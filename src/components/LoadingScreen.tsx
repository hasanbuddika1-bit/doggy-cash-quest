...
export function LoadingScreen({ progress }: LoadingScreenProps) {
  return (
    <div className="min-h-screen flex flex-col items-center justify-center bg-background relative overflow-hidden">
      ...
      >
        <div className="w-40 h-40 glow-gold rounded-full">
          <img src={bunnyLogo} alt="Bunny Earn Hub" className="w-full h-full object-contain drop-shadow-2xl" />
        </div>
        {[0, 60, 120, 180, 240, 300].map((deg, i) => (
          <motion.div
            key={i}
            className="absolute w-2 h-2 rounded-full bg-bunny-gold"
      ...
      <motion.h1
        ...
        className="text-3xl font-display font-bold text-3d-gold mt-6 z-10 text-center"
      >
        🐰 Bunny Earn Hub V2 ⛏️
      </motion.h1>
